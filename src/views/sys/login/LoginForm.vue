<template>
  ...
  >
    ...
    <n-form-item path="captcha">
      <n-space>
        <n-input v-model:value="formData.captcha" size="large" placeholder="请输入验证码" />
        <img v-if="picPath" :src="picPath" class="vPic" alt="请输入验证码" @click="loginVerify()" />
      </n-space>
    </n-form-item>
...
<script lang="ts" setup>
      ...
      try {
        ...
        if (userInfo) {
          notification?.success({
            ...
            content: `${t('sys.login.loginSuccessDesc')}: ${userInfo.userName}`,
            duration: 3000,
          });
        }
      } catch (error: any) {
  ...
  const loginVerify = async () => {
    ...
    formData.captchaId = ele.captchaId;
  };
  loginVerify();
</script>
<style>
